...
#from dashboard.models import BankAccount,Transaction
...
class EmailProcessor:
    ...
    def process_emails(self):
        ...
        for mensaje in email_resultados:
            ...
            # Verificar si los datos del pago son válidos
            if tag1 is not None and name1 is not None and amount1 is not None and received_date is not None:
                # Buscar la cuenta bancaria asociada a la etiqueta
                try:
                    bank_account = BankAccount.objects.get(etiqueta=tag1)
                except BankAccount.DoesNotExist:
                    print(f"La cuenta bancaria con etiqueta {tag1} no existe.")
                    continue

                # Verificar si el pago es un depósito o un retiro
                if amount1 > 0:
                    # Depósito
                    transaction_type = 'deposit'
                else:
                    # Retiro (el monto es negativo)
                    transaction_type = 'withdraw'

                # Crear la transacción y guardarla en la base de datos
                transaction = Transaction(bank_account=bank_account, amount=amount1, transaction_type=transaction_type)
                transaction.save()
