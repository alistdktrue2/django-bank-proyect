...
# Establece la variable de entorno DJANGO_SETTINGS_MODULE para apuntar a tu archivo de configuración de Django.
...
def mostrar_cuentas_aprobadas():
    cuentas_aprobadas = BankAccount.objects.filter(aprobado=True)
    
    if cuentas_aprobadas.exists():
        print("Cuentas Aprobadas:")
        for cuenta in cuentas_aprobadas:
            print(f"Nombre del Titular: {cuenta.titular}")
